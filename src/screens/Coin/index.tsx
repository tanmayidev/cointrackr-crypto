...
import { CoinFullInfo } from "../../types/Interface";
import { API_V3_URL } from "../../config/constants";

function Coin() {
  const { coinId } = useParams();
  const [coin, setCoin] = useState<CoinFullInfo>({});
  const [loadCoin, setLoadCoin] = useState(true);

  const url = `${API_V3_URL}/coins/${coinId}`;
            ...
            <div className="coin-content__text-side">
              <div className="numb">
                <div className="coin-content__text-side__24h">
                  ...
                  <p className={Number(h24) >= 0 ? "green-text" : "red-text"}>
                    {coin.market_data
